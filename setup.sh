...
cd "$SCRIPT_DIR/hackathon/starter"
npm install --silent 2>/dev/null
pass "Hackathon: Leaderboard starter deps installed"
...
if curl -sf http://localhost:3000/health > /dev/null 2>&1; then
  pass "Hackathon: Leaderboard responds on :3000"
else
  warn "Hackathon: Could not reach :3000 (port may be in use)"
fi
...
echo ""
echo "Quick start:"
echo "  Hackathon → cd hackathon/starter && npm start     # http://localhost:3000"
echo "  Spectator → open hackathon/spectator/big-screen.html in browser"
